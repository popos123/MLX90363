...
class Magnet {
  ...
  private:
    ...
    uint32_t send_last_time = 0, read_last_time = 0, current_read_time = 0, current_send_time = 0;
    bool ena_read = 1, ena_send = 0;
    ...
    float AngleDEC() {
      this->AngleLSB(); // get data to convert
  ...
  public:
    unsigned int Angle_LSB = 0;
    float Angle_DEC = 0;
    char Error_LSB = 0, Roll = 0, Gain = 0, CRC = 0;
    ...
    void ReadData() {
      if (ena_read == 1) {
        this->current_read_time = micros();
        if (current_read_time - read_last_time >= 100) { // minimum 100 us
          // Issue GET1 message
          ...
          for (int i = 0; i < 8; i++) {
            ...
            #endif
            if (i == 7) {
              this->send_last_time = micros(); // restart send_time
              digitalWrite(this->pin_SS1, HIGH); // Deselect
              #ifndef Software
              this->hspi->endTransaction();
              #endif
              this->ena_read = 0; // disable read_timer
              this->ena_send = 1; // enable send_timer
            }
          }
        }
      }
      if (ena_send == 1) {
        this->current_send_time = micros();
        if (current_send_time - send_last_time >= 700) { // minimum 700 us
          // Issue NOP message
          this->u8_spi_write_buffer[0] = 0x00;
          this->u8_spi_write_buffer[1] = 0x00;
          ...
          for (int i = 0; i < 8; i++) {
            ...
            #endif
            if (i == 7) {
              this->read_last_time = micros(); // restart read_time
              digitalWrite(this->pin_SS1, HIGH); // Deselect
              #ifndef Software
              this->hspi->endTransaction();
              #endif
              this->ena_read = 1; // enable read_timer
              this->ena_send = 0; // disable send_timer
              this->Angle_LSB = this->AngleLSB();
              this->Angle_DEC = this->AngleDEC();
              this->Error_LSB = this->ErrorBits();
              this->Roll = this->RollingCounter();
              this->Gain = this->VirtualGain();
              this->CRC = this->crc();
            }
          }
        }
